...
void reader(vector<long long> &read_req_time, vector<long long> &read_entry_time, int id)
{
    ...
    for (int var = 0; var < kr; var++)
    {
        ...
        // reqtime = max(reqtime , stringTolonglong(time_req));
        // cout << id << " " << var << " "<< reqtime << endl;
        ...
        // entrytime = max(entrytime , stringTolonglong(time_entry));
        // cout << id << " " << var << " "<< entrytime << endl;
    ...
    read_req_time[id] = reqtime / kr;
    read_entry_time[id] = entrytime / kr;
}
...
void writer(vector<long long> &write_req_time, vector<long long> &write_entry_time, int id)
{
    ...
    write_req_time[id] = reqtime / kw;
    write_entry_time[id] = entrytime / kw;
}
...
int main()
{
    ...
    outputFile.open("output-writer.txt"); // Open the output file
    ...
    sem_init(&write_lock, 0, 1);      // Initialize read lock semaphore with initial value 1
    sem_init(&read_try, 0, 1);        // Initialize queue lock semaphore with initial value 1
    ...
    outputFile.open("time-writer.txt");
    if (!outputFile.is_open())
    {
        cout << "Error opening output file" << endl;
        return 1;
